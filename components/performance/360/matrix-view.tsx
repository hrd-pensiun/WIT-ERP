...
const MOCK_EMPLOYEES = [
  { id: "1", label: "John Smith — Manager" },
  { id: "2", label: "Sarah Johnson — Developer" },
  { id: "3", label: "Michael Brown — Designer" },
]

const MOCK_PERIODS = ["Q2 2024", "Q1 2024", "Annual 2024"]
...
export function Matrix360View() {
  ...
  const [employee, setEmployee] = useState(MOCK_EMPLOYEES[0].id)
  const [period, setPeriod] = useState(MOCK_PERIODS[0])

  return (
    <Performance360Shell
      title="Dashboard"
      subtitle={`${scope.summaryLine} — Matrix & perhitungan skor (demo) · ${MOCK_EMPLOYEES.find((e) => e.id === employee)?.label ?? ""} · ${period}`}
    >
      <Card className="bg-slate-900 border-slate-800">
        <CardContent className="pt-6 space-y-6">
          ...
          <div className="border-t border-slate-800 pt-6 space-y-4">
            <h3 className="text-slate-200 font-medium">Pilih assessment</h3>
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div className="space-y-2">
                <Label className="text-slate-400 text-xs">Periode</Label>
