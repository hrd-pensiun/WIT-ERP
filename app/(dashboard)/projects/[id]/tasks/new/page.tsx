...
import { ArrowLeft, Save, Loader2, FileText, User, AlertCircle } from "lucide-react"
...
export default function NewProjectTaskPage() {
  ...
  const fetchUsers = useCallback(async () => {
    if (!insForge) return
    const { data } = await insForge.from("user_profiles").select("id,full_name,employee_number").is("deleted_at", null).order("full_name", { ascending: true })
  ...
  const handleSave = async () => {
    ...
    try {
      const { error: err } = await (insForge as any).from("project_tasks").insert([{
        project_id: id,
    ...
    <div className="space-y-5 max-w-3xl">
      ...
      <div className="flex items-start justify-between">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-zinc-900 dark:text-zinc-100">Task Baru</h1>
            <p className="text-xs text-zinc-400 mt-0.5">Tambah task ke project</p>
          </div>
        </div>
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 overflow-hidden">
        <div className="px-4 py-3 border-b border-zinc-100 dark:border-zinc-800 flex items-center gap-2">
          <span className="text-xs font-semibold text-zinc-600 dark:text-zinc-400">Jadwal & Estimasi</span>
        </div>
        <div>
          <div className={rowCls}>
