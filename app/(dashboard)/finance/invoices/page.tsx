    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 text-sm">Kelola data invoice pelanggan</p>
        </div>
      ...
      <Card className="bg-slate-900 border-slate-800">
        ...
        <CardContent>
          ...
          {loading ? (
            ...
            <div className="text-center py-10 text-slate-400">Belum ada invoice</div>
          ) : (
            <div className="space-y-3">
              {invoices.map((invoice) => (
                <div key={invoice.id} className="flex items-center justify-between rounded-lg border border-slate-800 bg-slate-950 p-4">
                  ...
                  <div className="flex items-center gap-2">
                    <Badge className="bg-slate-700 text-slate-200">{invoice.status}</Badge>
                    ...
                    <Button variant="ghost" size="icon" className="text-slate-400 hover:text-red-400" onClick={() => handleDelete(invoice.id)}>
