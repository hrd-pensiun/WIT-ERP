...
export default function GradeEditClient({ id }: { id: string }) {
  const router = useRouter()
  const { jobGrades, update, loading: hookLoading } = useJobGrades()
  ...
  useEffect(() => {
    ...
    if (grade) {
      setFormData({
        ...
        min_salary: grade.min_salary ? String(grade.min_salary) : "",
        max_salary: grade.max_salary ? String(grade.max_salary) : "",
      })
    }
  }, [jobGrades, id])
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div><h1 className="text-2xl font-bold text-slate-100">Edit Job Grade</h1><p className="text-xs text-slate-500">EDIT-JOB GRADE : {formData.name || formData.code || "-"}</p></div>
      </div>
      {error && <div className="bg-red-500/10 border border-red-500/20 text-red-400 px-4 py-3 rounded-lg">{error}</div>}
      <form onSubmit={handleSubmit}>
        <Card className="bg-slate-900 border-slate-800">
          <CardHeader><CardTitle className="text-slate-100 flex items-center gap-2"><Award className="w-5 h-5 text-emerald-500" /> Informasi Job Grade</CardTitle></CardHeader>
          <CardContent className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2"><Label className="text-slate-200">Gaji Minimum (Rp)</Label><Input type="number" value={formData.min_salary} onChange={(e) => setFormData({ ...formData, min_salary: e.target.value })} className="bg-slate-950 border-slate-800 text-slate-100" /></div>
              <div className="space-y-2"><Label className="text-slate-200">Gaji Maksimum (Rp)</Label><Input type="number" value={formData.max_salary} onChange={(e) => setFormData({ ...formData, max_salary: e.target.value })} className="bg-slate-950 border-slate-800 text-slate-100" /></div>
            </div>
