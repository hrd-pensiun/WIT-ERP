  ...
  useEffect(() => {
    ...
    if (lead) {
      setFormData({
        ...
        budget_value: (lead as any).budget_value ? String((lead as any).budget_value) : "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateLead(id, {
        ...
        budget_value: formData.budget_value || null,
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2"><Label>Estimasi Nilai (Rp)</Label><Input type="text" value={formData.estimated_value} onChange={(e) => setFormData({ ...formData, estimated_value: e.target.value, budget_value: e.target.value })} className="bg-background border-border text-foreground" /></div>
              <div className="space-y-2">
                ...
                <div className="space-y-2">
                  ...
                  {formData.budget_confirmed && (
                    <div className="relative">
                      <span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-xs text-zinc-400">Rp</span>
                      <input
                        type="text"
                        value={formData.budget_value}
                        onChange={(e) => setFormData({ ...formData, budget_value: e.target.value, estimated_value: e.target.value })}
                        placeholder="Nilai budget..."
                        className="w-full h-8 pl-8 pr-2 text-xs bg-background border border-border rounded-md text-foreground placeholder:text-zinc-400 focus:outline-none focus:ring-1 focus:ring-emerald-500/50"
                      />
                    </div>
                  )}
                </div>
